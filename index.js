...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const convertedLowerCase = arr.map(function(item){
...
function lowPopulationAnimals(arr){
  const lowPopulation = arr.filter(function(item){
...
function USApop(arr){
  const totalPopulation = arr.reduce(function(acc, i){
...
const someCube = new CubeMaker({
  side: 3,
})

console.log("CUBEMAKER: ", someCube.volume());
console.log("CUBEMAKER: ", someCube.surfaceArea());
